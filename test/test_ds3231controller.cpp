...
void test_format_day_mask_weekdays(void) {
    ...
    // Should contain Mon, Tue, Wed, Thu, Fri
    TEST_ASSERT_TRUE(result.indexOf("Mon") >= 0);
    TEST_ASSERT_TRUE(result.indexOf("Fri") >= 0);
    TEST_ASSERT_TRUE(result.indexOf("Sun") < 0);  // Should not contain Sunday
    TEST_ASSERT_TRUE(result.indexOf("Sat") < 0);  // Should not contain Saturday
}

void test_format_day_mask_all_days(void) {
    String result = DS3231Controller::formatDayMask(0b01111111);  // All days
    TEST_ASSERT_TRUE(result.indexOf("Sun") >= 0);
    TEST_ASSERT_TRUE(result.indexOf("Sat") >= 0);
}
