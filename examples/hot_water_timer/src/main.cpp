...
// Forward declarations (required when compiling as .cpp instead of .ino)
void setupSchedules();
void activateHeater();
...
void loop() {
    ...
    if (rtc.isAlarmFired(1)) {
        ...
        // Without a next alarm the timer stops firing entirely, so report it.
        if (!rtc.setAlarmForNextSchedule()) {
            Serial.println("Failed to arm next schedule alarm");
        }
    }
...
void setupSchedules() {
    ...
    if (!rtc.addSchedule(morningWeekday)) {
        Serial.println("Failed to add schedule: morningWeekday");
    }
    ...
    if (!rtc.addSchedule(eveningWeekday)) {
        Serial.println("Failed to add schedule: eveningWeekday");
    }
    ...
    if (!rtc.addSchedule(morningWeekend)) {
        Serial.println("Failed to add schedule: morningWeekend");
    }
    ...
    if (!rtc.addSchedule(eveningWeekend)) {
        Serial.println("Failed to add schedule: eveningWeekend");
    }
}
    ...
    switch (cmd) {
        ...
        case '1'...'7': {
            ...
            if (schedule) {
                ...
                if (!rtc.updateSchedule(scheduleId, *schedule)) {
                    Serial.printf("Failed to update schedule %d\n", scheduleId);
                }
