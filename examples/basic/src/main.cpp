...
void setup() {
    ...
    // Set alarm for next schedule. A silently failed alarm means the schedule
    // never fires, so report it rather than discarding the result.
    if (!rtc.setAlarmForNextSchedule()) {
        Serial.println("Failed to set alarm for next schedule");
    }
